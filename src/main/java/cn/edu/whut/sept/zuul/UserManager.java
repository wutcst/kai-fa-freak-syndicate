...
public class UserManager {
  ...
  private static final String DB_URL =
      "jdbc:mysql://localhost:3306/zuul_game?useSSL=false&serverTimezone=UTC";
  ...
  public String login(String username, String password) {
    ...
    try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
      ...
      if (rs.next()) {
        ...
        User user =
            new User(
                rs.getString("username"), rs.getString("password"), true // 密码已加密
                );
  ...
  public void updateGameData(int score) {
    ...
    String sql =
        "UPDATE users SET highest_score = GREATEST(highest_score, ?), "
